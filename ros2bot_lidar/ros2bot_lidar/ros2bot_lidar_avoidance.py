...
class Ros2botLidarAvoidanceNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('laser_angle', 30) # 10-180
    ...
    def laser_scan_cb(self, msg):
        ...
        for i in range(len(ranges)):
            ...
            if 160 > angle > 180 - self.laser_angle:
                if ranges[i] < self.response_dist: self.right_warning += 1
            if - 160 < angle < self.laser_angle - 180:
                if ranges[i] < self.response_dist: self.left_warning += 1
            if abs(angle) > 160:
                if ranges[i] <= self.response_dist: self.front_warning += 1
